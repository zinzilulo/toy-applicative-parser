...
mod parser_demo {
    ...
    fn cmd<'a>() -> Parser<'a, Maybe<Command>> {
        ...
        let b_head = then_keep_left::<P<'a>, _, _>(
    ...
    pub fn run() {
        ...
        let multi_digit = liftA2::<P<'_>, _, _, _>(&digit, &digit, |x, y| (x, y));
        ...
        let many_digit_values = parse(
            &many::<P<'_>, _>(&fmap::<P<'_>, _, _>(&digit, |c: char| -> u32 {
...
where {
    ...
    pub fn run() {
        let item = satisfy(|_| true);
        let item_tests = vec![runParser(&item, ""), runParser(&item, "a1")];
        println!("{item_tests:?}");
        ...
        let full_tests = vec![runParser(&full, "hihello"), runParser(&full, "hellohi")];
        println!("{full_tests:?}");
        ...
        let hello_then_bang = then_keep_left::<Parser<'_, String, Maybe<String>>, _, _>(
            &string_("hello"),
            &char_('!'),
        );
        ...
        let fmap_tests = vec![runParser(&fmap_left, "1"), runParser(&fmap_right, "1")];
        println!("{fmap_tests:?}");
    }
}
